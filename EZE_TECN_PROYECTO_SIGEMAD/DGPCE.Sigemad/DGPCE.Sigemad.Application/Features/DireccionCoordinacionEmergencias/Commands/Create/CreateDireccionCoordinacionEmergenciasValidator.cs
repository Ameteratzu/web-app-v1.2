...
public class CreateDireccionCoordinacionEmergenciasValidator : AbstractValidator<CreateDireccionCoordinacionEmergenciasCommand>
{
    public CreateDireccionCoordinacionEmergenciasValidator(IStringLocalizer<ValidationMessages> localizer)
    {
        ...
        //RuleFor(p => p.GeoPosicionPMA)
        //    .Must(GeoJsonValidatorUtil.IsGeometryInWgs84).WithMessage(localizer["GeoPosicionPMAInvalida"]);

        //RuleFor(p => p.GeoPosicionCECOPI)
        // .Must(GeoJsonValidatorUtil.IsGeometryInWgs84).WithMessage(localizer["GeoPosicionCECOPOInvalida"]);
